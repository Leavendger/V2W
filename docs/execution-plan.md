...
### 任务清单
- [ ] `app.py` 新增 `GET /search` 路由：文件名查询 + 转写查询（join，按文件分组）+ `tab` 参数 + 三类计数 + `highlight` 过滤器
- [ ] `templates/search.html` 结果页：Tab 栏（全部/项目名/转文字）+ 项目名命中块 + 内容命中块 + 空态
- [ ] `templates/base.html` 导航栏全局搜索框
...
### 任务清单
- [ ] `utils.py` 新增 `speaker_label()`（统一占位「发言人」，预留 speaker 扩展点）+ `segments_to_markdown()`
- [ ] `app.py` 新增 `GET /file/<id>/export`：生成 md + 中文文件名 RFC 5987 编码下载
- [ ] `templates/detail.html` 已完成文件显示「导出 Markdown」按钮
- [ ] `static/style.css` 导出按钮（描边样式）
