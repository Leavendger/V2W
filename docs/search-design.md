...
> 在转写文本与项目名中快速定位关键内容，并跳转播放。本文档为后续迭代的实现依据。
>
> 对应执行计划：**P6（单文件内搜索）→ P7（全局搜索）**，详见 [execution-plan.md](execution-plan.md)。
>
> 全局搜索（P7）覆盖两个维度：**项目名**（文件名）与**转写文字**，结果分「全部 / 项目名 / 转文字」三类 Tab。
...
| **阶段二** | 全局跨文件搜索（项目名 + 转写文字） | 导航栏搜索框 | **P7** | 「这句话是哪场会议说的」/「那个项目叫什么来着」 |
...
全局搜索除转写文字外，还匹配 **文件名**（`File.filename`），覆盖**所有状态**的文件（含未转写完成的——刚上传就能按名找到）：
...
转写命中仅限 `status == 'completed'`；项目名命中不限状态。两类独立查询，结果在「全部」Tab 去重并集。
...
### 5.2 阶段二 — 全局搜索（项目名 + 转写文字）

```
GET /search?q=<关键词>&tab=all|name|content
```

服务端渲染 HTML 结果页，同时计算三类计数：

- `tab=all`（默认）：项目名命中 + 转写命中，**分两块**展示
- `tab=name`：仅项目名命中的文件列表（文件名高亮 + 状态 + 时长）
- `tab=content`：仅转写命中，按文件分组（文件名 + 时间戳 + 高亮片段 + 深链跳转）

模板变量：`name_files`（项目名命中文件）、`content_groups`（转写命中分组）、`name_count` / `content_count` / `total_count`（去重并集），以及当前 `tab`。关键词高亮由 Jinja `highlight` 过滤器（服务端 `<mark>`）完成。

> 深链：转写命中条目链接 `/file/<id>?q=<kw>&seg=<segment_index>`，详情页进入即自动定位（见 [第 8.2 节](#82-全局搜索结果页与三类-tab)）。
...
导航栏全局搜索框（所有页面可见，`GET /search`，关键词回填）。单输入框，分类在结果页用 Tab 切换。
...
- **Tab 切换**：链接 `/search?q=&tab=`，服务端渲染，URL 可分享 / 刷新保持；
- **「全部」分两块**：上方「项目名命中」、下方「内容命中」，各管各的（同一文件两类都命中时各出现一次）；
- **项目名命中条目**：文件图标 + 文件名（关键词高亮）+ 状态标签 + 时长，点击进详情页；
- **内容命中**：「→跳转」链接 `/file/<id>?q=<kw>&seg=<segment_index>`；
- **详情页读取 `q` / `seg`**：进入即自动填入搜索框、定位到指定命中段，复用阶段一全部逻辑；
- **空状态**：全无命中显示「未找到」；当前 Tab 分类无结果（他类有）显示「该分类下无结果」。
...
### 10.2 P7 — 全局搜索（项目名 + 转写文字）

1. `app.py` 新增 `GET /search`（HTML）路由：文件名查询 + 转写查询（join，避免 N+1，按文件分组）+ `tab` 参数 + 三类计数（去重并集）+ Jinja `highlight` 过滤器（服务端高亮）；
2. `templates/search.html` 结果页：Tab 栏 + 项目名命中块 + 内容命中块 + 空 / 分类空态；
3. `templates/base.html` 导航栏全局搜索框（关键词回填）；
4. `templates/detail.html` 支持 `?q=&seg=` 深链定位。

**验收：**

- [ ] 项目名命中：文件名含关键词的文件列出（含未转写完成的），文件名关键词高亮；
- [ ] 转写命中：跨文件列出命中片段，按文件分组；
- [ ] Tab 切换（全部 / 项目名 / 转文字）计数正确、当前高亮；
- [ ] 「全部」分两块（项目名命中 + 内容命中）；
- [ ] 点击转写结果 → 跳详情页并自动定位高亮关键词；
- [ ] 全无命中 / 某分类无结果 两种空状态正确。
